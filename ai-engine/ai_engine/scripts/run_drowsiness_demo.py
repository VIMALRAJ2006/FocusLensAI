#!/usr/bin/env python3
"""
Task 03 demo — face mesh + eye tracking + drowsiness detection.
...
from ai_engine.config.thresholds import DrowsinessThresholds  # noqa: E402
from ai_engine.detection.drowsiness_detector import DrowsinessDetector  # noqa: E402
...
def main() -> int:
    ...
    drowsiness = DrowsinessDetector(DrowsinessThresholds())
    ...
    fps_smooth = 0.0
    print("Drowsiness demo running. Press Q to quit.")
    print(
        "Close eyes briefly to blink; hold closed to trigger DROWSY "
        f"(>{DrowsinessThresholds().drowsiness_min_consecutive_frames} frames)."
    )
    ...
    try:
        while True:
            ...
            state = drowsiness.update_from_eye(eye)
            annotated = drowsy_renderer.draw(eye_renderer.draw(frame, eye), state)
            ...
            if state.is_drowsy:
                header = "DROWSY"
            elif mesh.face_present and eye:
                header = f"FACE | EAR {eye.ear:.3f}"
            ...
            cv2.imshow("FocusLens — Drowsiness (Task 03)", annotated)
